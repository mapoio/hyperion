...
func NewViperProvider(configPath string) (*ViperProvider, error) {
	...
	return &ViperProvider{
		v:         v,
		callbacks: make([]func(ChangeEvent), 0),
	}, nil
}
...
// Note: The ChangeEvent.Key will be empty and Value will be nil for file-based
// watches. Use the Provider methods to read the updated configuration values.
//
...
func (p *ViperProvider) Watch(callback func(event ChangeEvent)) (stop func(), err error) {
	...
	if len(p.callbacks) == 1 {
		...
		p.v.OnConfigChange(func(e fsnotify.Event) {
			p.mu.RLock()
			callbacks := make([]func(ChangeEvent), len(p.callbacks))
	...
	// Return stop function that removes this specific callback
	callbackIndex := len(p.callbacks) - 1
	return func() {
		p.mu.Lock()
		defer p.mu.Unlock()

		// Remove callback by replacing it with the last one and truncating
		if callbackIndex < len(p.callbacks) {
			p.callbacks = append(p.callbacks[:callbackIndex], p.callbacks[callbackIndex+1:]...)
		}
	}, nil
}
